...
fn parse(input: &str) -> Grid<usize> {
    Grid::new(
        input
            .lines()
            .map(|line| line.chars().map(|v| v as usize - '0' as usize).collect::<Vec<_>>())
            .collect::<Vec<_>>()
    )
}
...
fn part_a(g: Grid<usize>) -> usize {
    ...
    for y in 1..bounds.1-1 {
        for x in 1..bounds.0-1 {
            ...
            let right = (x..bounds.0).rev().map(|xx| *g.at(xx, y)).collect::<Vec<_>>();
            let top = (0..=y).map(|yy| *g.at(x, yy)).collect::<Vec<_>>();
            let bottom = (y..bounds.1).rev().map(|yy| *g.at(x, yy)).collect::<Vec<_>>();

            let sides = vec![left, right, top, bottom];

            let result = sides.iter().map(|side| {
